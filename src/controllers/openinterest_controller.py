...
async def get_fno_price_data(
    ...
    try:
        ...
        data = await openinterest_service.get_fno_price_data(date=date, instrument_id=instrument_id)
        ...
        series_data = await openinterest_service.extract_series_from_response(data)
...
async def get_fno_data_custom(
    ...
    **additional_params
    ...
    try:
        ...
        logger.info(f"🎯 OpenInterest custom FNO data request with params: {params}")

        data = await openinterest_service.get_fno_data_with_custom_params(**params)
        ...
        logger.error(f"❌ Error in OpenInterest custom FNO data endpoint: {e}")
        raise HTTPException(status_code=500, detail=str(e))
